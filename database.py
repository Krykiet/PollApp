...
SELECT_POLL_VOTE_DETAILS = """SELECT
    ...
    COUNT(votes.option_id) / SUM(COUNT(votes.option_id)) OVER() * 100.0 AS vote_percentag
FROM options
...
def create_poll(connection, title: str, owner, options: List[str]):
    with connection:
        with connection.cursor() as cursor:
            # cursor returns ID of created poll
            cursor.execute(INSERT_POLL_RETURN_ID, (title, owner))

            poll_id = cursor.fetchone()[0] # fetching id of the poll
            option_values = [(option_text, poll_id) for option_text in options]
            # psycopg2 helper to avoid multiple executes, takes list of tuples
